...
def get_tests_file_name(name, directory=None):
    tests = []
    if directory is None:
        directory = base
    else:
        directory = os.path.join(base, directory)
    sys.path.insert(0, directory)
    compiler = os.path.join(directory, "compiler_ut")
    sys.path.insert(0, compiler)
    if testfile.match(name):
        modname = os.path.splitext(name)[0]
        if modname in imported:
            print(
                f"Test module '{modname}' imported both as '{imported[modname]}' and "
...
if __name__ == "__main__":
    ...
    if DebugConfig().get_debug_mode():
        ...
        elif DebugConfig().get_debug_reslter():
            path_name = os.path.join(os.getcwd(), "restler_ut")
            tests = get_tests(path_name)
        else:
            sys.exit(-1)
