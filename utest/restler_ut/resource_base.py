...
class ResourceBase:
    ...
    @abstractmethod
    def set_resource(self, data_type: str, name: str, body: str = None):
    ...
    def delete_resource(self, data_type: str, name: str):
        """ Attempts to delete a specific child resource

        @param data_type: The resource type to delete
        @param name: The resource name to delete

        @return: None

        """
        if data_type in self._children and \
                name in self._children[data_type]:
            del self._children[data_type][name]
        else:
    ...
    def get_resource_object(self, objects: list):
        ...
        if obj_type in self._children and \
                self._children[obj_type] is not None \
                and dyn_object in self._children[obj_type]:
            # Walk the tree
            return self._children[obj_type][dyn_object].get_resource_object(objects[2:])
        else:
            raise ResourceDoesNotExist

    def get_data(self, data_type: str = None) -> dict:
        """ Returns data from a specified resource or resource group

        @param data_type: The 'type' of the resource to get data from.
        @param data_type: The 'name' of the individual resource to get data from.
        ...
        @return: The data from the resource or resource type
                 - If getting from a type/resource group, return all the dynamic object data of that type.
                 - If getting a resource, return all of it's data

        """
        if data_type:
            if data_type in self._children:
                # If just getting from a type, return dyn objects of this type
                ret_list = {data_type: dict()}
                if self._children[data_type] is not None:
                    for child in self._children[data_type]:
                        ret_list[data_type][child] = self._children[data_type][child].data
                return ret_list
            else:
                raise ResourceDoesNotExist
        else:
