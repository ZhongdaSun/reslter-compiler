...
import rest.restler.utils.formatting as formatting

from rest.restler.engine import bug_bucketing
from rest.restler import checkers, restler_settings
from rest.restler.restler_settings import (
    Settings,
    RestlerSettings,
    LogSetting)
import rest.restler.engine.dependencies as dependencies
import rest.restler.engine.core.preprocessing as preprocessing
import rest.restler.engine.core.postprocessing as postprocessing
import rest.restler.engine.core.driver as driver
import rest.restler.engine.core.fuzzer as fuzzer
import rest.restler.utils.import_utilities as import_utilities
import rest.restler.engine.core.fuzzing_monitor as fuzzing_monitor
import rest.restler.engine.core.requests as requests
from rest.restler.engine.errors import InvalidDictionaryException
from rest.restler.engine.errors import NoTokenSpecifiedException
from rest.restler.engine.primitives import InvalidDictPrimitiveException
from rest.restler.engine.primitives import UnsupportedPrimitiveException
...
class ExecuteParam:
    ...
    """
    host: str
    usepath_regex: str_test_socket: str
    test_server: str
    set_version: str
    : str
...
def load_common_engine():
    ...
    engine_file = os.path.join(base_folder, "restler_user_settings.json")
    try:
    ...
    if "custom_mutations" in engine_settings_dict.keys():
        engine_settings_dict["custom_mutations"] = os.path.join(base_folder, engine_settings_dict["custom_mutations"])

    if "custom_value_generators" in engine_settings_dict.keys():
        engine_settings_dict["custom_value_generators"] = os.path.join(base_folder,
    ...
    if "checkers" in engine_settings_dict.keys():
        ...
        for key_item, values in checkers_value.items():
            if "custom_mutations" in values.keys():
                values["custom_mutations"] = os.path.join(base_folder, values["custom_mutations"])
            if "custom_value_generators" in values.keys():
                values["custom_value_generators"] = os.path.join(base_folder, values["custom_value_generators"])
...
def execute_restler(config_arg: ExecuteParam):
    ...
    if bool(config_arg.settings):
        try:
            with open(config_arg.settings, encoding='utf-8') as file_handler:
                engine_settings_file = json.load(file_handler)
                file_handler.close()
        except Exception as error:
            print(f"Error: Failed to load settings file: {error!s}")
            sys.exit(-1)

        engine_file_dir = os.path.dirname(config_arg.settings)
    ...
    if engine_settings_file:
        local_user_args.update(engine_settings_file)
    ...
    if config_arg.custom_mutations:
        try:
            if os.path.exists(config_arg.custom_mutations):
                custom_mutations_dict = json.load(open(config_arg.custom_mutations, encoding='utf-8'))
            elif os.path.exists(os.path.join(engine_file_dir, config_arg.custom_mutations)):
                custom_mutations_dict = json.load(open(os.path.join(engine_file_dir, config_arg.custom_mutations),
                                                       encoding='utf-8'))
            else:
                custom_mutations_dict = json.load(open(os.path.join(os.path.join(__file__), config_arg.custom_mutations),
                                                       encoding='utf-8'))
        except Exception as error:
    ...
    if config_arg.replay_log:
        replay_valid_extensions = [".replay.txt", ".ndjson"]
        if not config_arg.replay_log.endswith(tuple(replay_valid_extensions)):
    ...
    if config_arg.replay_log and config_arg.replay_log.endswith(".replay.txt"):
        try:
            logger.create_network_log(logger.LOG_TYPE_REPLAY)
            driver.replay_sequence_from_log(config_arg.replay_log, engine_settings.token_refresh_cmd)
...
def execute_from_command_line():
    ...
    try:
        req_collection.set_custom_mutations(custom_mutations, per_endpoint_custom_mutations,
                                            settings.custom_value_generators_file_path)
    except UnsupportedPrimitiveException as primitive:
        logger.write_to_main("Error in mutations dictionary.\n"
                             f"Unsupported primitive type defined: {primitive!s}",
                             print_to_console=LogSetting().restler)
