...
from rest.restler.engine.transport_layer.response import CONNECTION_CLOSED_CODE
from rest.restler.engine.transport_layer.response import TIMEOUT_CODE
...
class BugBuckets(object):
    ...
    def _test_bug_reproducibility(self, sequence, bug_code, bucket, candidate_values_pool):
        ...
        """
        logger.write_to_main("_test_bug_reproducibility", True)
    ...
    def update_bug_buckets(self, sequence, bug_code, origin='main_driver', reproduce=False, additional_log_str=None,
                           checker_str=None, hash_full_request=False, lock=None):
        ...
        try:
            ...
            logger.update_bug_buckets(self._bug_buckets, sent_request_data_list, bug_hash,
                                      additional_log_str=additional_log_str)
        finally:
            if lock is not None:
                lock.release()
