...
from rest.restler.restler_settings import TokenAuthMethod
...
from rest.restler.engine.transport_layer.response import RESTLER_BUG_CODES
from rest.restler.engine.transport_layer.messaging import UTF8
from rest.restler.engine.transport_layer.messaging import HttpSock
from rest.restler.engine.core.retry_handler import RetryHandler
from rest.restler.utils import import_utilities
...
def resolve_dynamic_primitives(values, candidate_values_pool):
    ...
    for i in range(len(values)):
        ...
        if isinstance(values[i], tuple) \
            ...
            quoted = False
            if len(current_uuid_type_name) >= 2 and current_uuid_type_name[0] == '"' and current_uuid_type_name[
                -1] == '"':
                quoted = True
...
#def send_request_data(rendered_data, req_timeout_sec=None):
def send_request_data(rendered_data, req_timeout_sec=None, reconnect=None, http_sock=None):
    ...
    while num_retries < MAX_RETRIES:
        ...
        return response
    """
    from restler.engine.transport_layer.restclient import RestClient

    root_api_url = 'http://192.168.1.21:17103'
    rest_client = RestClient(api_root_url=root_api_url)
    ...
    while num_retries < MAX_RETRIES:
        ...
        if num_retries > 0:
            reconnect = True
        req_timeout_sec = Settings().max_request_execution_time if req_timeout_sec is None else req_timeout_sec
        success, response = rest_client.rest_client_method(rendered_data)
...
def call_response_parser(parser, response, request=None, responses=None):
    ...
    """
    from rest.restler.utils.restler_logger import write_to_main
...
def _AUTH_LOGGING(log_str):
    from rest.restler.utils.restler_logger import auth_logging as AUTH_LOGGING
