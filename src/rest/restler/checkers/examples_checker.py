...
from rest.restler.checkers.checker_base import *
from rest.restler.engine.fuzzing_parameters.fuzzing_utils import *
...
from rest.restler.engine.core.sequences import Sequence
...
class ExamplesChecker(CheckerBase):
    ...
    def apply(self, rendered_sequence, lock):
        ...
        logger.write_to_main(f"_sent_request_data_list={self._sequence._sent_request_data_list}", True)
        ...
        if last_request_def in self._tested_requests:
            logger.write_to_main("exit.......", True)
            return

        self._tested_requests.add(last_request_def)

        if last_request.examples:
            logger.write_to_main(f"last_request.examples={last_request.examples}", True)
    ...
    def _send_each_example(self, request):
        ...
        for example in filter(lambda x: x is not None, request.examples.body_examples):
            ...
            self._log("blocks={}, fuzzing_config={}".format(blocks, fuzzing_config))
        ...
        for example in filter(lambda x: x is not None, request.examples.query_examples):
            ...
            for idx, query in enumerate(example.param_list):
                self._log("example.para_list={}, fuzzing_config={}".format(example.param_list, fuzzing_config))
        ...
        for example in filter(lambda x: x is not None, request.examples.header_examples):
            self._log("example.para_list={}".format(example.param_list))
