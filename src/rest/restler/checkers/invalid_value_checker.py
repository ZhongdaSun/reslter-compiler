...
from rest.restler.engine.primitives import CandidateValuesPool
    ...
    def init_mutations(self):
        ...
        try:
            if invalid_mutations_file_path is None:
                ...
                with open(invalid_mutations_file_path, encoding='utf-8') as file_handler:
                    self._custom_invalid_mutations = json.load(file_handler)
                    file_handler.close()
        ...
        if self._value_generators_file_path is None:
            ...
            self._value_generators_file_path = os.path.join(Settings().engine_file_folder,
    ...
    def apply(self, rendered_sequence, lock):
        ...
        for idx, is_fuzzable in enumerate(fuzzable_parameter_value_blocks):
            ...
            self._checker_log.checker_print(
                "fuzzable_parameter_value_blocks={}".format(fuzzable_parameter_value_blocks))
            ...
            try:
                ...
                for fuzzed_value in tv:
                    ...
                    logger.write_to_main(f"response.status={response.status_code}, "
                                         f"response.to_str={response.to_str}", True)
