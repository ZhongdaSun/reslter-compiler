...
from rest.restler.checkers.body_schema_fuzzer import *
from rest.restler.checkers.response_analyzer import ResponseTracker
from rest.restler.checkers.payload_body_bucketing import *
        ...
        for rendered_data, parser, _, updated_writer_variables, replay_blocks in new_request.render_iter(
            ...
            if self._rule_violation(seq, response, valid_is_violation):
                ...
                if bug_info is not None:
                    ...
                    logger.write_to_main(f"{self.__class__.__name__}", True)
                    BugBuckets.Instance().update_bug_buckets(
                        err_seq, response.status_code, origin=self.__class__.__name__, checker_str=error_str,
