...
from rest.compiler.grammar import (
    ...
    OperationMethod,
    RequestId)
from rest.compiler.example import ExampleRequestPayload
...
class ProducerIndexes:
    ...
    indexed_by_endpoint: {}  # Dict[RequestId, List[ResponseProducer]]
    ...
    def __dict__(self):
        ...
        if len(self.same_payload_producers) > 0:
            ...
            for keys, values in self.same_payload_producers.items():
                ...
                for item in values:
                    by_endpoint_list.append(item.__dict__())
                same_payload_dict["ResponseProducer"] = by_endpoint_list
...
class Producers:
    ...
    def add_response_producer(self, resource_name, producer: Producer):
        logger.write_to_main(f"request_id={producer.request_id.endpoint}, "
                             f"resource_name={resource_name}, "
                             f"producer={producer.__dict__()}", ConfigSetting().LogConfig.dependencies_analysis_type)
        resource_producers = self.try_add(resource_name)
        sort_key = inferred_match_sort(producer)
        ...
        logger.write_to_main(f"sort_key={sort_key}", ConfigSetting().LogConfig.dependencies_analysis_type)
        ...
        logger.write_to_main(f"end request_id={producer.request_id.endpoint}, "
                             f"resource_name={resource_name}, "
                             f"producer={resource_producers.__dict__()}",
                             ConfigSetting().LogConfig.dependencies_analysis_type)
...
class PathPart:
    ...
    def __init__(self, part_type: PathPartType, value: str):
        logger.write_to_main(f"part_type={part_type}, value={value}",
                             ConfigSetting().LogConfig.dependencies_analysis_type)
...
class Path:
    ...
    def get_path_parts_before_parameter(self, name):
        ...
        logger.write_to_main(f"path_until_parameter={path_until_parameter}",
                             ConfigSetting().LogConfig.dependencies_analysis_type)
...
def get_path_from_string(path, include_separators):
    ...
    for i, p in enumerate(parts):
        ...
        logger.write_to_main(f"sub_parts={sub_parts}", ConfigSetting().LogConfig.dependencies_analysis_type)
