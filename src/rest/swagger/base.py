...
from rest.swagger.utils import jp_compose
...
class Context(object):
    ...
    @classmethod
    def is_produced(cls, obj):
        """ Provide a hook for customizing ways to check produced output
        """
        return isinstance(obj, cls.__swagger_ref_object__)
    ...
    def parse(self, obj=None):
        ...
        def _apply(x, kk, ct, v):
            if key not in self._obj:
                self._obj[kk] = {} if ct is None else []
        ...
        if hasattr(self, '__swagger_child__'):
            ...
            for key, (ct, ctx_kls) in six.iteritems(self.__swagger_child__):
                ...
                # logger.info("key = {}, items={}".format(key, items))
...
class BaseObj(object):
    ...
    def _assign_parent(self, ctx):
        ...
        def _assign(cls, _, obj):
            ...
            if cls.is_produced(obj):
                ...
                raise ValueError('Object is not instance of {0} but {1}'.format(cls.__swagger_ref_object__.__name__,
    ...
    def merge(self, other, ctx, exclude=[]):
        ...
        :Param Array exclude: exclude the parameters.
        """
...
class FieldMeta(type):
    ...
    def __new__(metacls, name, bases, spc):
        ...
        """
        logger.write_to_main("fields={}, spc={}".format(name, spc), False)

        def init_fields(fields, rename):
            logger.write_to_main("fields={}, rename={}".format(fields, rename), False)
        ...
        def _default_(name, default):
            logger.write_to_main("name={}, default={}".format(name, default), False)
