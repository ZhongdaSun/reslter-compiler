...
from rest.swagger.utils import final, deref, CaseInsensitiveDict
...
class Request(object):
    ...
    def _prepare_forms(self):
        ...
        if self.__op.consumes and content_type not in self.__op.consumes:
            raise TypeError('content type {0} does not present in {1}'.format(content_type, self.__op.consumes))
    ...
    def _prepare_body(self):
        ...
        if self.__op.consumes and content_type not in self.__op.consumes:
            raise ValueError('content type {0} does not present in {1}'.format(content_type, self.__op.consumes))
        ...
        for parameter in self.__op.parameters:
            ...
            if getattr(parameter, 'in') == 'body':
                ...
                return content_type, self.__op._mime_codec.marshal(content_type, body, _type=_type, _format=_format,
                                                                   name=name)
        return None, None
    ...
    def prepare(self, scheme='http', handle_files=True, encoding='utf-8'):
        ...
        if isinstance(scheme, list):
            if self.__scheme is None:
                ...
                if self.__scheme in scheme:
                    ...
                    raise Exception(
                        'preferred scheme:{} is not supported by the client or spec:{}'.format(self.__scheme, scheme))
        ...
        path_params = {}
        logger.info("enter prepare,{}".format(self.__p))
        ...
        if accept:
            if self.__op.produces and accept not in self.__op.produces:
                raise ValueError('accept {0} does not present in {1}'.format(accept, self.__op.produces))
