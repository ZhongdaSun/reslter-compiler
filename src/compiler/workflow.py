...
from compiler.example import get_example_config_file
from compiler.compiler import generate_request_grammar
from compiler.restler_engine_settings import update_engine_settings
...
def generate_grammar_from_swagger(grammar_output_directory_path: str):
    ...
    def write_dictionary(dictionary_mame, new_dict_mutation, is_merged: bool):
        ...
        if is_merged:
            ...
            new_dict_mutation = current_dict
        JsonSerialization.serialize_to_file(new_dictionary_file_path, new_dict_mutation.__dict__())

    # todo
    write_dictionary(Constants.NewDictionaryFileName, new_dictionary, True)
    ...
    if per_resource_dictionaries and len(per_resource_dictionaries) > 1:
        for dict_name, dict_contents in per_resource_dictionaries.items():
            write_dictionary(f"{dict_contents[0][0]}.json", dict_contents[0][1], False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--api_spec', help='API Spec File ame',
                        type=str, default=None, required=False)
    parser.add_argument('--config', help='API Spec config',
                        type=str, default=None, required=False)
        ...
        if os.path.exists(args.api_spec):
            ...
            if ConfigSetting().GrammarOutputDirectoryPath == "":
                ConfigSetting().GrammarOutputDirectoryPath = (
                    convert_to_abs_path(
                        os.path.join(os.path.dirname(args.api_spec), args.api_spec.rsplit(".json", 1)[0]),
                        Constants.DefaultGrammarOutputDirectory))
