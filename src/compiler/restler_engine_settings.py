...
class EngineSettings:
    ...
    def add_per_resource_timing_delays(self, requests):
        ...
        for req in requests:
            if req.requestMetadata.is_long_running_operation:
    ...
    def add_per_resource_dictionaries(self, per_resource_dictionaries, requests):
        per_resource_settings = self.get_per_resource_settings() or {}
        for req in requests:
            if req.id.hex_hash in per_resource_dictionaries.keys():
                values = per_resource_dictionaries[req.id.hex_hash]
                ...
                if req.id.endpoint not in per_resource_settings:
                    per_resource_settings[req.id.endpoint] = dictionary_setting
                else:
                    per_resource_settings[req.id.endpoint].setdefault(Constants.CustomDictionaryKey,
    ...
    def __dict__(self):
        dict_value = {
            ...
            "max_combinations": self.settings.get(Constants.MaxParameterCombinationsKey)
        }
        """
        dict_value = {
...
def update_engine_settings(requests,
    ...
    if engine_settings_file_path is not None:
        ...
        if is_succeeded:
            engine_settings.add_per_resource_timing_delays(requests)
            engine_settings.add_per_resource_dictionaries(per_resource_dictionaries,requests)
