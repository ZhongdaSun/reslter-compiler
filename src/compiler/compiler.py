                    ...
                    if parameter.type == "array":
                        ...
                        if property_payload is not None:
                            if isinstance(property_payload, LeafNode):
                                ...
                                if isinstance(fp, FuzzingPayload):
...
def get_injected_header_or_query_parameters(request_id: RequestId,
    ...
    def is_request_specific_name(name: str) -> bool:
        # Request-specific custom payloads are of the form <endpoint>/<method>/<parameter name>
        """
        Checks if the given `name` string follows the request-specific custom payload format:
        <endpoint>/<method>/<parameter name>.

        :param name: The input string to check.
        :param supported_methods: A list of supported operation methods (e.g., ['get', 'post', 'put', 'delete']).
        :return: True if the name follows the required format, False otherwise.
        """
        ...
        return any(f"/{method}/" in name_lower for method in SupportedOperationMethods)
...
def generate_request_header_primitives(request_parameters: RequestParameters,
    ...
    if len(request_parameters.header) == 0:
        ...
        for payload_source, request_headers in request_parameters.header:
            ...
            parameter_list = [rp for rp in parameter_list if rp.name != "Content-Length" and rp.name != "Content-Type"]
...
def generate_request_query_primitives(request_parameters: RequestParameters,
    ...
    if request_query_parameters is None or len(request_query_parameters) == 0:
        ...
        if len(injected_custom_payload_query_parameters) > 0:
            return [(ParameterPayloadSource.DictionaryCustomPayload,
                     ParameterList(injected_custom_payload_query_parameters))]
        else:
            return [(ParameterPayloadSource.Schema,
                     ParameterList([]))]
    elif (len(request_query_parameters) == 1 and request_query_parameters[0][0] == ParameterPayloadSource.Schema and
