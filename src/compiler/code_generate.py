...
class NameGenerators:
    ...
    def generate_producer_endpoint_response_parser_function_name(request_id):
        return f"parse_{DynamicObjectNaming.generate_dynamic_object_variable_name(request_id, None, "")}"
...
def format_restler_fuzzable_object(primitive_type: RequestPrimitiveType):
    ...
    tracked_param_name = get_tracked_param_primitive_parameter(
        primitive_type.primitive_data.tracked_parameter_name)
    if primitive_type.dynamic_object:
        writer = generate_writer_statement(primitive_type.dynamic_object)
        if example_parameter or tracked_param_name:
...
def get_restler_python_payload(payload, is_quoted):
    def get_primitive_payload(p):
            ...
            if payload.primitive_type == PrimitiveType.Bool:
                ...
                if isinstance(payload.default_value, PrimitiveTypeEnum):
                    ...
                    request_primitive_type = RequestPrimitiveTypeData(default_value=group_value,
                                                                      ...
                                                                      example_value=exv,
                                                                      tracked_parameter_name=parameter_name)
                    return RequestPrimitiveType.fuzzable_group(data=request_primitive_type,
                                                               dynamic_object=dynamic_object)
        ...
        elif type(payload) is PayloadPartsPayload:
            raise Exception("Expected primitive payload")

    if isinstance(payload, PayloadPartsPayload):
        # Assuming parts is a list
        result_list = []
        for part in payload.payload_value:
            result_list.extend(get_restler_python_payload(part, False))
        return result_list
    else:
        return [get_primitive_payload(payload)]
...
def get_parameter_list_payload(parameters: list[tuple[ParameterPayloadSource, RequestParametersPayload]]) -> (
    ...
    if len(injected_payload) == 0:
        ...
        if len(declared_payload) == 0:
            return declared_payload_source, injected_payload
        else:
            return declared_payload_source, declared_payload + injected_payload
    ...
    if request_element[0] == RequestElementType.Method:
        ...
        if len(value) > 0:
            ...
            for payload_parts in value:
                result.extend(get_restler_python_payload(payload_parts, False))
            return result
        else:
            return [RequestPrimitiveType.static_string_constant(value='/')]
        ...
        if request_element[1]:
            filter_out = []
            for parameter_source, parameter_lists in request_element[1]:
        ...
        if request_element[1]:
            parameter_source, parameter_lists = request_element[1][0], request_element[1][1]
            for parameter_list in parameter_lists:
                if isinstance(parameter_list, ParameterList):
                    parameters = []
                    ...
                    raise UnsupportedType(
                        f"This request parameters payload type is not supported: {type(parameter_list)}")
        return []
                ...
                if isinstance(parameter_list, ParameterList):
                    ...
                    for request in parameter_list.request_parameters:
                        parameters = parameters + generate_python_parameter(parameter_source,
                                                                            ParameterKind.Body,
                                                                            request)
                    return parameters + [RequestPrimitiveType.static_string_constant(r"\r\n")]
                elif isinstance(parameter_list, CustomPayload):
                    return (get_restler_python_payload(parameter_list, False)
                            + [RequestPrimitiveType.static_string_constant(r"\r\n")])
                else:
                    raise UnsupportedType(
                        f"This request parameters payload type is not supported: {request_element[0]}")
        ...
        if request_element[1]:
            ...
            writer_variable_statements = [
                ...
                for producer_writer in (variables_referenced_in_parser + request_element[1].input_writer_variables)
            ]
...
def get_response_parsers(requests: List[Request]):
    ...
    for d in dependency_data:
        variables = get_dynamic_object_definitions(d.input_writer_variables)
        for item in variables:
            if item is not None:
...
def generate_python_from_request(request: Request, merge_static_strings):
    ...
    index = 0
    for item in request.headerParameters:
        return_value = get_parameter_list_payload(item)
        for payload_source_, payload_value in item:
            if return_value is not None:
                if payload_source_ == ParameterPayloadSource.DictionaryCustomPayload:
                    if index == 0:
                        request_header_elements.append(return_value)
                        index = 1
                    else:
                        request_header_elements.insert(0, return_value)
                else:
                    request_header_elements.append(return_value)
                    break
    ...
    for request_element in request_elements:
        ...
        if request_element[0] == RequestElementType.Body and merge_static_strings and len(primitives) > 1:
            filtered_primitives = []
            last_element = primitives[len(primitives) - 1]
            ...
            processed_elements.append(last_element)
        else:
        ...
        if '_fuzzable_' in name:
            ...
            if custom_payloads is not None:
                ...
                for cp in custom_payloads:
                    filtered_name = DynamicObjectNaming.generate_python_function_name_from_string(cp, "_")
                    function_name = "gen_" + name + "_" + filtered_name
