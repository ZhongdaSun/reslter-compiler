...
class PayloadPartsPayload:
    payload_value: list[Union[Constant, FuzzablePayload, CustomPayload, DynamicObject]]
    ...
    def __dict__(self):
        dict_value = []
        for item in self.payload_value:
            dict_value.append(item.__dict__())
        return {"PayloadParts": dict_value}
...
FuzzingPayload = Union[Constant, FuzzablePayload, CustomPayload, DynamicObject, PayloadPartsPayload]
...
class Request:
    ...
    def query_body_to_json(self, parameters, use_example: bool):
        ...
        if parameters is not None:
            for parameter in parameters:
                ...
                elif parameter[0] == ParameterPayloadSource.DictionaryCustomPayload:
                    dictionary_dict_list = {"ParameterList": parameter[1].__dict__()}
        ...
        if self.id.has_example and use_example:
            ...
            return return_value
        if dictionary_dict_list is not None:
            return_value.append(("DictionaryCustomPayload", dictionary_dict_list))
            return return_value
    ...
    def header_to_json(self, param):
        ...
        dict_content_type = []
        return_value = []
        for index, parameter in enumerate(param):
            for item in parameter:
                if item[0] == ParameterPayloadSource.Schema:
                    ...
                    if index == 0:
                        ...
                        dict_content_type = dict_content_type + item[1].__dict__()
        ...
        if ConfigSetting().UseHeaderExamples:
            if ((ConfigSetting().UseQueryExamples or ConfigSetting().UseBodyExamples) and self.id.has_example
                ...
                if len(schema_dict_list) > 0 or (len(schema_dict_list) == 0 and len(dict_list) == 0):
                    param_schema = {"ParameterList": schema_dict_list}
                    return_value.append(("Schema", param_schema))
        else:
            if len(schema_dict_list) > 0 or (len(schema_dict_list) == 0 and len(dict_list) == 0):
        ...
        param_dict = {"ParameterList": dict_content_type}
        return_value.append(["DictionaryCustomPayload", param_dict])
...
class DynamicObjectNaming:
    ...
    @staticmethod
    def generate_python_function_name_from_string(func_str, delimiter):
        # pattern = "|".join(map(re.escape, DynamicObjectNaming.ReplaceTargets))
        endpoint_parts = re.split('|'.join(map(re.escape, DynamicObjectNaming.ReplaceTargets)), func_str)
        return delimiter.join(endpoint_parts)
