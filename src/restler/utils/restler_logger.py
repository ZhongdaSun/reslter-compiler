...
from restler.engine import dependencies as dependencies
from restler.engine import primitives as primitives
from restler.utils import formatting as formatting

ALREADY_CREATED_ENVIRONMENT = False
...
REQUEST_RENDERING_LOGS = ""
# GC tests logs.
GARBAGE_COLLECTOR_LOGS = ""
# Experiment logs dir
LOGS_DIR = ""
    ...
    def generate_summary_speccov(self):
        ...
        try:
            with open(file_path, encoding='utf-8') as file_handler:
                full_speccov = json.load(file_handler)
                file_handler.close()
        ...
        for key, group in itertools.groupby(full_speccov.items(), get_verb_endpoint):
            ...
            new_speccov[k] = v
        with open(new_file_path, 'w', encoding='utf-8') as file_handler:
            json.dump(new_speccov, file_handler, indent=4)
            file_handler.close()
...
def create_experiment_dir(logger_path):
    """ creates the unique EXPERIMENT_DIR directory where results are saved
    @return: None    @rtype:
    """
    global ALREADY_CREATED_ENVIRONMENT
    if ALREADY_CREATED_ENVIRONMENT:
        return
    ...
    if SETTINGS_SAVE_RESULTS_IN_FIXED_DIRNAME:
        ...
        adder = 0
        while True:
            EXPERIMENT_DIR = os.path.join(logger_path, 'RestlerResults', f'experiment{(os.getpid() + adder)!s}')
    ...
    ALREADY_CREATED_ENVIRONMENT = True
...
def remove_tokens_from_logs(data):
    ...
    """
    global SETTINGS_NO_TOKENS_IN_LOGS
    from restler.engine.core.request_utilities import replace_auth_token
...
def write_to_main(data: str, print_to_console: bool = False):
    ...
    log_info = f"[Function: {info.function}] [File: {os.path.basename(info.filename)}]  [Line: {info.lineno}]"  # [{formatting.timestamp()}]
    # if Settings().disable_logging:
    #    return
    try:
        main_lock.acquire()
        with open(MAIN_LOGS, "a+", encoding='utf-8') as f:
            print(log_info + data, file=f)
            f.flush()
...
def raw_network_logging(data):
    """ Helper to log network traffic transferred over sockets.

    @param data: The data payload to be logged.
    @type data: Str
...
# Dict to track whether a bug was already logged:
...
def update_bug_buckets(bug_buckets, bug_request_data, bug_hash, additional_log_str=None):
    ...
    def log_new_bug():
        ...
        with open(filepath, "w+", encoding='utf-8') as bug_file:
            ...
            print(" If an authentication token is required, you must also specify the token_refresh_cmd.",
                  file=bug_file)
    ...
    def log_new_bug_as_json():
        ...
        filepath = os.path.join(BUG_BUCKETS_DIR, filename)
        current_sequence = bug_bucket.sequence

        bug_detail = BugDetail()
        bug_detail.checker_name = bug_bucket.origin
        bug_detail.reproducible = bug_bucket.reproducible
        bug_detail.endpoint = current_sequence.last_request._endpoint_no_dynamic_objects
        bug_detail.verb = current_sequence.last_request.method
        sequence_request_counter = 0
        for req in bug_request_data:
            try:
                bug_request = BugRequest()
                bug_request.replay_request = req.rendered_data
                bug_request.response = req.response
                bug_request.producer_timing_delay = req.producer_timing_delay
                bug_request.max_async_wait_time = req.max_async_wait_time
                bug_detail.request_sequence.append(bug_request)
                if sequence_request_counter == len(bug_request_data) - 1:
                    if len(req.response.split(DELIM)) > 1:
                        split_response_body = req.response.split(DELIM)
                        response_headers = split_response_body[0].split("\r\n")
                        response_status_code_and_status_message = response_headers[0].split(" ")
                        bug_detail.status_code = response_status_code_and_status_message[1]
                        bug_detail.status_text = ' '.join(response_status_code_and_status_message[2:])
                    else:
                        bug_detail.status_code = ''
                        bug_detail.status_text = ''
            ...
            for seq_hash in bug_buckets[bucket_class]:
                ...
                if bucket_hash not in Bugs_Logged:
                    try:
                        ...
                        request_bug = Bug()
                        request_bug.filepath = filenameJson
                        request_bug.reproducible = bug_bucket.reproducible
                        request_bug.checker_name = bug_bucket.origin
                        request_bug.error_code = bug_bucket.error_code

                        write_incremental_bugs(os.path.join(BUG_BUCKETS_DIR, "Bugs.json"), request_bug)
                        Bugs_Logged[bucket_hash] = BugTuple(filename, bug_hash, bug_bucket.reproduce_attempts,
                                                            bug_bucket.reproduce_successes)
                        add_hash(filename)
                    except Exception as error:
                    ...
                    Bugs_Logged[bucket_hash] = BugTuple(filename, this_bug_hash, bug_bucket.reproduce_attempts,
                                                        bug_bucket.reproduce_successes)
...
def print_req_collection_stats(req_collection, candidate_values_pool):
    ...
    val = statistics.mean([r.num_combinations(candidate_values_pool) for r in req_collection])
    data += f"{timestamp}: Avg. Value Combinations per Request: {val}\n"

    val = statistics.median([r.num_combinations(candidate_values_pool) for r in req_collection])
...
def print_generation_stats(req_collection, fuzzing_monitor, global_lock, final=False):
    ...
    """
    from restler.engine.bug_bucketing import BugBuckets
    from restler.engine.transport_layer.response import VALID_CODES
    from restler.engine.transport_layer.response import RESTLER_INVALID_CODE
    timestamp = formatting.timestamp()
    ...
    write_to_main(
        ...
        f"{timestamp}: Num invalid requests caused by failed resource creations: "
        f"{num_invalid_by_failed_resource_creations}\n"
...
def format_request_block(request_id, request_block, candidate_values_pool):
    ...
    # Handle custom payload
        ...
        values = candidate_values_pool.get_candidate_values(primitive, request_id=request_id, tag=current_fuzzable_tag,
                                                            quoted=quoted)
    ...
    # Handle all the rest
        ...
        values = candidate_values_pool.get_fuzzable_values(primitive, default_val, request_id, quoted=quoted,
                                                           examples=examples)
...
def format_rendering_stats_definition(request, candidate_values_pool, log_file=None, log_all_fuzzable_values=False):
    print_data = get_rendering_stats_definition(request, candidate_values_pool, log_file=log_file,
                                                log_all_fuzzable_values=log_all_fuzzable_values)
...
def print_request_rendering_stats(candidate_values_pool, fuzzing_requests, fuzzing_monitor, num_rendered_requests,
    ...
    with open(REQUEST_RENDERING_LOGS, "a+", encoding='utf-8') as log_file:
        print(f"\n{timestamp}: {generation_name}"
              ...
              f"\n{timestamp}: \tRendered requests with \"valid\" status codes: "
              f"{sum(successful_requests)} / {num_rendered_requests}"
              f"\n{timestamp}: \tRendered requests determined to be fully valid (no resource creation failures): "
              f"{fully_valid_count} / {num_rendered_requests}",
              file=log_file)

        # if all request have succeed, we don't need longer generations
...
def print_request_coverage(request=None, rendered_sequence=None, log_rendered_hash=True):
    ...
    @param request: The rendered request    @type  request:

    @param rendered_sequence: The rendered sequence
    @type  rendered_sequence: RenderedSequence

    @param log_rendered_hash: The rendered hash
    @type  log_rendered_hash: bool
