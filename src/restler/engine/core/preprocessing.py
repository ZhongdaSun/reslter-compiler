...
import restler.engine.core.fuzzing_requests as fuzzing_requests
...
from restler.engine.fuzzing_parameters.request_examples import *
from restler.engine.fuzzing_parameters.body_schema import *
from restler.engine.fuzzing_parameters.parameter_schema import *
...
def create_fuzzing_req_collection(path_regex):
    ...
    for request in GrammarRequestCollection():
        ...
        logger.write_to_main(f"include_req={include_req}", True)
        ...
        if include_req:
            ...
            if request.is_resource_generator():
                included_requests_list.insert(0, request)
            else:
                included_requests_list.append(request)
        else:
    ...
    for request in included_requests_list:
        reqs = driver.compute_request_goal_seq(
            request, GrammarRequestCollection())
        for req in reqs:
            if req not in included_requests_list:
                if req.is_resource_generator():
                    included_requests_list.index(0, req)
                else:
                    included_requests_list.append(req)
            else:
                if req.is_resource_generator():
                    included_requests_list.remove(req)
                    included_requests_list.insert(0, req)
    ...
    if Settings().in_smoke_test_mode():
        fuzz_reqs.set_all_requests(included_requests_list)
        return fuzz_reqs
    else:
...
def _set_method_endpoint_schemas(examples: RequestExamples, body_schema: BodySchema, query_schema: QueryList,
    ...
    if hex_def in request_collection:
        ...
        # logger.warning("Error not found in the request collection")
    ...
    for endpoint in create_once_endpoints:
        ...
        if endpoint in GrammarRequestCollection().request_id_collection:
            ...
            for req in GrammarRequestCollection().request_id_collection[endpoint]:
                logger.write_to_main("req.endpoint={}, req.method={}".format(req.endpoint, req.method), True)
