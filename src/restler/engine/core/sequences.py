...
import restler.engine.core.async_request_utilities as async_request_utilities
import restler.engine.core.request_utilities as request_utilities
import restler.engine.core.status_codes_monitor as status_codes_monitor
...
from restler.utils.restler_logger import custom_network_logging as CUSTOM_LOGGING
    ...
    def render(self, candidate_values_pool, lock, preprocessing=False, postprocessing=False):
        ...
        for rendered_data, parser, tracked_parameters, updated_writer_variables, replay_blocks in \
            ...
            if response.has_bug_code():
                logger.write_to_main(f"{self.__class__.__name__}", LogSettings().sequences)
                BugBuckets.Instance().update_bug_buckets(
