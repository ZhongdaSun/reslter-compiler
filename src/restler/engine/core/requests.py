...
from restler.engine.fuzzing_parameters.request_examples import RequestExamples
from restler.engine.fuzzing_parameters.body_schema import BodySchema
from restler.engine.fuzzing_parameters.parameter_schema import QueryList
from restler.engine.fuzzing_parameters.parameter_schema import HeaderList
import restler.engine.fuzzing_parameters.param_combinations as param_combinations
from restler.engine.fuzzing_parameters.fuzzing_config import FuzzingConfig
...
import restler.engine.mime.multipart_formdata as multipart_formdata
...
class Request(object):
    ...
    def is_resource_generator(self):
        ...
        if 'POST' in self.method or 'PUT' in self.method or 'GET' in self.method:
            if bool(self.metadata) and 'post_send' in self.metadata:
    ...
    def init_fuzzable_values(self, req_definition, candidate_values_pool, preprocessing=False,
        ...
        for request_block in req_definition:
            ...
            # Handle multipart form data
                ...
                try:
                    current_fuzzable_values = candidate_values_pool. \
                        get_candidate_values(primitive_type, request_id=self._request_id, tag=default_val,
                                             quoted=quoted)
            ...
            # Handle all the rest
                ...
                values = candidate_values_pool.get_fuzzable_values(primitive_type, default_val, self._request_id,
                                                                   quoted, examples)
    ...
    def render_iter(self, candidate_values_pool, skip=0, preprocessing=False, prev_rendered_values=None,
        ...
        def _handle_exception(type, tag, err):
            logger.write_to_main(
                ...
                f"Type: {type}. Tag: {tag}.\n"
                f"  Exception: {err!s}",
                print_to_console=LogSettings().requests
            )
            raise InvalidDictionaryException
    ...
    def substitute_headers(self, new_header_blocks):
        ...
        if header_end_index < 0:
            logger.write_to_main(f"could not get start and end of header. header_end_index: {header_end_index}", True)
            return None
    ...
    def get_query_param_combinations(self, query_param_combinations_setting):
        ...
        for param_list in param_combinations.get_param_combinations(self, query_param_combinations_setting,
            ...
            if new_request:
                ...
                logger.write_to_main(f"Warning: could not substitute query parameters.", True)
    ...
    def get_header_param_combinations(self, header_param_combinations_setting):
        ...
        for param_list in param_combinations.get_param_combinations(self, header_param_combinations_setting,
            ...
            if new_request:
                ...
                # For malformed requests, it is possible that the place to insert parameters is not found,
                # In such cases, skip the combination.
                logger.write_to_main(f"Warning: could not substitute header parameters.", True)
    ...
    def get_example_payloads(self):
        ...
        def check_example_schema_is_valid(num_payloads, total_payloads, payload_kind):
            ...
            if not is_valid:
                ...
                logger.write_to_main(error_message, True)
...
class RequestCollection(object):
    ...
    def add_request(self, request):
        ...
        """
        logger.warning("request={}".format(request.request_id))
    ...
    def set_custom_mutations(self, custom_mutations, per_endpoint_custom_mutations,
                             value_generators_file_path, per_endpoint_custom_generators):
        ...
        @param value_generators_file_path: the path of generators.
        @type value_generators_file_path: str
        @param per_endpoint_custom_generators: The per-endpoint custom generators
        @type per_endpoint_custom_generators: Dict
        ...
        if per_endpoint_custom_generators:
            self.candidate_values_pool.set_per_value_generators(per_endpoint_custom_generators)
