...
class CandidateValuesPool(object):
    ...
    def __init__(self):
        ...
        self.per_endpoint_value_generators = {}
        ...
        for primitive in custom_mutations:
            ...
            if grammar_primitive in self.supported_primitive_dict_types:
                if not isinstance(custom_mutations[primitive], dict):
                    raise InvalidDictPrimitiveException(
                        f'primitive: {primitive}, type: {type(custom_mutations[primitive])}')
                ...
                current_primitives[grammar_primitive] = _assign_values(current_primitives[grammar_primitive],
                                                                       custom_mutations[primitive])
    ...
    def get_candidate_values(self, primitive_name, request_id=None, tag=None, quoted=False, examples=[]):
        ...
        if request_id and request_id in self.per_endpoint_value_generators:
            candidate_value_gen = None
            value_generators = self.per_endpoint_value_generators[request_id]
            if primitive_name in value_generators:
                if tag:
                    if tag in value_generators[primitive_name]:
                        candidate_value_gen = value_generators[primitive_name][tag]
                else:
                    candidate_value_gen = value_generators[primitive_name]
    ...
    def set_per_value_generators(self, per_value_generators):
        for request_id, generator in per_value_generators.items():
            if os.path.exists(generator):
                attrs = import_utilities.import_attrs(generator, ["value_generators", "set_random_seed"])
                self.per_endpoint_value_generators[request_id] = attrs[0]
            else:
