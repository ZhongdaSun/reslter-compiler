...
class SettingsArg(object):
    ...
    def __init__(self, name, type, default, user_args, minval=None, min_exactok=True, maxval=None, max_exactok=True,
                 val_convert=None):
...
class SettingsListArg(SettingsArg):
    ...
    def __init__(self, name, type, user_args, minval=None, min_exactok=True, maxval=None, max_exactok=True,
                 val_convert=None):
        ...
        """
        super(SettingsListArg, self).__init__(name, type, None, None, minval=minval, min_exactok=min_exactok,
                                              maxval=maxval, max_exactok=max_exactok)
        self.val = []
        self.val_convert = val_convert
        self._set_arg(user_args)
...
class SettingsDictArg(SettingsArg):
    ...
    def __init__(self, name, type, minval=None, min_exactok=True, maxval=None, max_exactok=True, val_convert=None,
        ...
        """
        super(SettingsDictArg, self).__init__(name, type, None, None, minval=minval, min_exactok=min_exactok,
                                              maxval=maxval, max_exactok=max_exactok)
        self.val = dict()
...
class RestlerSettings(object):
    ...
    def __init__(self, user_args, engine_file_folder):
        ...
        if RestlerSettings.__instance:
            logging.warning("RestlerSettings.__instance={}".format(type(RestlerSettings.__instance)))
            raise NewSingletonError("Attempting to create a new singleton instance.")

        from rest.restler.engine.core.request_utilities import str_to_hex_def
        ...
        self._custom_bug_codes = SettingsListArg('custom_bug_codes', re.Pattern, user_args,
                                                 val_convert=convert_wildcards_to_regex)
        ...
        self._custom_generators = SettingsDictArg('custom_generators', str, key_convert=str_to_hex_def)
        ## List of status codes that represent "non-bugs". All other status codes will be treated as bugs.
        self._custom_non_bug_codes = SettingsListArg('custom_non_bug_codes', re.Pattern, user_args,
                                                     val_convert=convert_wildcards_to_regex)
        ...
        self._dyn_objects_cache_size = SettingsArg('dyn_objects_cache_size', int, DYN_OBJECTS_CACHE_SIZE_DEFAULT,
                                                   user_args, minval=0)
        ...
        # # Length of time the garbage collector will attempt to cleanup remaining resources at the end of fuzzing (
        # seconds)
        self._garbage_collector_cleanup_time = SettingsArg('garbage_collector_cleanup_time', int,
                                                           MAX_GC_CLEANUP_TIME_SECONDS_DEFAULT, user_args, minval=0)
        ...
        self._max_objects_per_resource_type = SettingsArg('max_objects_per_resource_type', int, None, user_args,
                                                          minval=0)
        ...
        self._max_async_resource_creation_time = SettingsArg('max_async_resource_creation_time', (int, float),
                                                             MAX_ASYNC_RESOURCE_CREATION_TIME_DEFAULT, user_args,
                                                             minval=0)
        ...
        self._max_request_execution_time = SettingsArg('max_request_execution_time', (int, float),
                                                       MAX_REQUEST_EXECUTION_TIME_DEFAULT, user_args, minval=0,
                                                       min_exactok=False, maxval=MAX_REQUEST_EXECUTION_TIME_MAX)
        ## Maximum length of any sequence
        self._max_sequence_length = SettingsArg('max_sequence_length', int, MAX_SEQUENCE_LENGTH_DEFAULT, user_args,
                                                minval=0)
        ...
        self._resource_producer_timing_delays = SettingsDictArg('per_resource_producer_timing_delay', int,
                                                                key_convert=str_to_hex_def)
        ...
        self._max_logged_request_combinations = SettingsArg('max_logged_request_combinations', int, None, user_args,
                                                            minval=0)
        ...
        self._engine_file_folder = SettingsArg('engine_file_folder', str, 'engine_file_folder', user_args)
    ...
    @property
    def engine_file_folder(self):
        print(f"self._engine_file_folder={type(self._engine_file_folder.val)}")
        return self._engine_file_folder.val
    ...
    def trace_db_cleanup_time(self):
        if 'cleanup_time' in self._trace_db_settings.val:
            ...
            if isinstance(cleanup_time, int) and cleanup_time >= 0:
                ...
                raise ValueError(
                    "Invalid value for 'cleanup_time'. It should be an integer greater than or equal to 0.")
        else:
    ...
    def trace_db_custom_serializer_file_path(self):
        ...
        if not os.path.exists(custom_serializer_file_path):
            raise ValueError(
                f"The specified path for the custom serializer module ({custom_serializer_file_path}) does not exist.")
    ...
    def include_request(self, endpoint, method):
        ...
        def contains_request(req_list):
            for req in req_list:
                # print("file={}, req={}, endpoint={}, method={}".format(__file__, req, endpoint, method))
    ...
    def _set_per_resource_args(self, args: dict):
        ...
        for endpoint in args:
            ...
            if self._custom_generators.name in args[endpoint]:
                custom_generators[endpoint] = args[endpoint][self._custom_generators.name]

        self._resource_producer_timing_delays.set_val(timing_delays)
        self._custom_dictionaries.set_val(custom_dicts)
        self._custom_generators.set_val(custom_generators)
    ...
    def in_smoke_test_mode(self) -> bool:
        ...
        """
        return ((self._fuzzing_mode.val == 'directed-smoke-test' or self._fuzzing_mode.val == 'test-all-combinations')
                and (not self.in_scenario_replay_mode()))
    ...
    def get_endpoint_custom_generators_paths(self) -> dict:
        """ Returns the dict containing the endpoint specific custom mutations

        @return: The endpoint specific custom mutations dict

        """
        return self._custom_generators.val
        ...
        def validate_auth_options():
            ...
            if len(user_provided_token_auth_options) > 1:
                raise OptionValidationError(
                    f"Must specify only one token authentication mechanism - received {user_provided_token_auth_options}")
